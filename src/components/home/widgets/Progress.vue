<template>
    <v-card class="mx-2" fill outlined>
        <v-card-title class="text-subtitle-1">
            {{ showTextByType }}毎の実施頻度進捗
            <v-spacer />
            <v-btn dark text color="primary" @click="type = (type == 'year' ? 'month' : 'year')">
...
export default {
    ...
    methods: {
        async fetchData() {
            ...
            // パーセント計算
            this.series = [Math.round((this.count / this.countDaysOfThisMonthOrYear) * 100)];
        },
    ...
    computed: {
        ...
        countDaysOfThisMonthOrYear() {
            return this.type == 'year' ? (this.$moment().endOf('year').dayOfYear()) : this.$moment().daysInMonth()
        },
        todayYear() {
        ...
        showTarget() {
            return this.type == 'year' ? this.todayYear : this.todayYear + this.todayMonth;
        }
    }
}
</script>